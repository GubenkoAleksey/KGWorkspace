    ...
    fun showReminderNotification(title: String, message: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            if (ContextCompat.checkSelfPermission(context, android.Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
                Log.e(TAG, "Notification permission (POST_NOTIFICATIONS) is not granted! Cannot show notification: $title")
                return
            }
        }
        ...
        // Use a default Android icon since we don't have a specific app icon yet
        ...
        try {
            ...
            Log.d(TAG, "Notification successfully sent to NotificationManager")
        } catch (e: Exception) {
